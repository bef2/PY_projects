...
class excelWriter:
    ...
    try:
        ...
        try:
            ...
            pass
        pass
    file_name = 'MTM9D ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())[:10].replace('-', '.') + '_' + \
                         time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())[11:].replace(':', '.')
    ...
    def write(self,row, line_data):
        ...
        self.excel_sheet[f'E{row}'] = line_data['values']
...
class serial_485_stream(QThread):
    ...
    def run(self):
        ...
        while self.flag_state:
            send_command = "001M^\r"    # Команда: Чтение измерения давления
            try:
            ...
            try:
                data_formatted = f"{int(dataIn[4:8]) * 1 / 10 ** (23 - int(dataIn[8:10])) * 0.75}"
...
class myWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle(f"Манометр {excelWriter.file_name}")
        
        self.frame_remPanel = QtWidgets.QFrame(self)
        self.frame_remPanel.setMinimumWidth(530)
        self.frame_remPanel.setMaximumWidth(530)
