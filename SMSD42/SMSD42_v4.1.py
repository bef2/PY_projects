...
#------------------------------------------------------------------------------------------
        ...
        self.var['frame_setCOM_' + axis] = QFrame(self.var['frame_axis_' + axis])

        # Надпись - имя оси
        self.var['label_axis_' + axis] = QLabel(f'{axis.upper()}', self.var['frame_axis_' + axis])
        self.var['label_axis_' + axis].setFont(QFont("Arial", 16))
        self.var['label_axis_' + axis].setStyleSheet(f"color: rgb{TEXT_AXIS};")

        # Выпадающий список портов
        self.var['combobox_setCOM_' + axis] = QComboBox(self.var['frame_setCOM_' + axis])
        ...
        # self.var['button_setCOM_' + axis].clicked.connect(self.openCOM)
